...
#       p2 = 150GB  inference model storage
...
# nvme-362935: inference OS (80GB) + model storage (150GB)
...
if [ -b "$INF_DRIVE" ]; then
    ...
    sudo parted -s "$INF_DRIVE" mkpart primary ext4  1MiB  80GiB   # inference-0 OS
    sudo parted -s "$INF_DRIVE" mkpart primary ext4 80GiB 230GiB   # model storage
    sleep 2
else

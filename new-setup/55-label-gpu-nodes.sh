...
# Optional per-node overrides (export before running if needed)
INFERENCE_0_GPU_COUNT="${INFERENCE_0_GPU_COUNT:-}"
INFERENCE_1_GPU_COUNT="${INFERENCE_1_GPU_COUNT:-}"
...
detect_gpu_count() {
  ...
  # Fallback to per-node overrides or default
  case "$node" in
    inference-0)
      if [ -n "${INFERENCE_0_GPU_COUNT}" ]; then echo "${INFERENCE_0_GPU_COUNT}"; return 0; fi
      ;;
    inference-1)
      if [ -n "${INFERENCE_1_GPU_COUNT}" ]; then echo "${INFERENCE_1_GPU_COUNT}"; return 0; fi
      ;;
  esac
